...
//        System.out.println("당신의 이름은 " + name + "입니다.");
        ...
        /*
            ...
            이름을 입력하세요 >>> 여러분이름
            주소를 입력하세요 >>> 여러분 주소(시 / 구까지 혹은 도 / 시까지)
            나이를 입력하세요 >>> 여러분나이
...
//        String address = scanner.nextLine();
        /*
            ...
            36라인의 nextInt() 작성후에 enter를 때려버리면 38라인에서 enter가 지 건줄 알고
            그대로 소화해버려서 아무 값도 입력되지 않게 됩니다.

            문제를 해결하기 위한 방식은 아래에 작성합니다.
         */
        System.out.print("이름을 입력하세요 >>> ");
        String name = scanner.nextLine();
        System.out.print("나이를 입력하세요 >>> ");
        int age = scanner.nextInt();
        scanner.nextLine(); // 50번 라인에서의 엔터키를 막아주는 배리어 역할
        ...
        /*
            next() vs. nextLine()
                ...
                2. 1.로 인한 문제점 : next() 다음에 nextLine()을 혼합하여 쓰게 될 때,
                    nextLine()을 호출하기 전에 next()를 먼저 사용하게 되면, next() 입력하고 난 후에 친
                    enter키로 인해 nextLine()이 입력을 무시하고 다음 코드라인을 읽어들이면서 빈 문자열만 저장,
                    변수에 아무 것도 대입되지 않을 수 있음.
